...
if ($loja == 'todos'){
		$sql =  "
				select * from (	select ve.nome, sum((nvl(ca.total,0)- nvl(ca.iva01val,0) - nvl(iva02val,0) - nvl(iva03val,0) - nvl(iva04val,0))*decode(ca.tipodoc,'DCL',-1,1)) soma
								...
								group by  ve.nome
								order by soma desc, ve.nome asc )
				where rownum <= 10";
		...
		$sql =  "
				select * from (	select   ve.nome, sum((ca.total)*decode(ca.tipodoc,'DCL',-1,1)) soma 
								from vendas_vdcab ca, vendas_vditem it,  ctb_vendedor ve, dash_lojas dl 
								where ca.codemp 		= ".$_SESSION['codemp']."
									/*and ca.tipodoc  	<> 'DCL' 
									and ca.tipodoc  	<> 'FSC' 
