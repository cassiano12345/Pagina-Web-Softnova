...
*******************************************************************************************************************************************************/
	...
	$content = '
		<ul class="breadcrumb">
			...
			<li><a href="#">Mov. de Contas Gerais</a></li>
		</ul>
		<div id="contentMain">
			<div class="box span12">
				<div class="box-header" data-original-title="">
					<h2><i class="halflings-icon edit"></i><span class="break"></span>Mov. de Contas Gerais</h2>
				</div>
							...
							<fieldset>
								<div id="ProcurarArtigo">
                                	<div class="control-group" id="pesquisas">
										<label class="control-label" for="loja">Tipo de Movimento</label>
										<div class="controls">
											<select id="tipoPesquisa" >
												<option value="2">Todos Movimentos</option>
                                                <option value="1">Movimentos em Aberto</option>
											</select>
										</div>
									</div>
									...
									<div class="control-group">
										...
										<div class="control-group">
											<label class="control-label" for="typeahead"> Conta Geral </label>
											<div class="controls">
												<input autocomplete="off" class="span6 typeahead " style="width:100%;" placeholder=" Pesquisar Conta Geral" id="Conta_Geral" type="number" >
											</div>							
										</div>
...
$script="<script type='text/javascript'>
    ...
    $('#Conta_Geral').on('input', function() {
        if ($('#Conta_Geral').val().length >= 2) {
            var expr = $('#Conta_Geral').val();
			var fina = $('#datafinal').datepicker('option', 'dateFormat', 'dd/mm/yy').val();
			var ano = fina.split('/')[2];
            
            $.getJSON('modules/contabilidade/Lista_CG_contabilidade.php', {'exp': expr, 'data': ano }, function(infoArtigo) {
                $('#Conta_Geral').data('typeahead').source = infoArtigo;
            });
        } else {
            $('#Conta_Geral').data('typeahead').source = [];
        }
    });
});
	...
	$(document).ready(function () {
		...
		var output_INI = (day<10 ? '0' : '') + day+ '/' +(month<10 ? '0' : '') + (month - 1) + '/' + d.getFullYear();
		var output = (day<10 ? '0' : '') + day+ '/' +(month<10 ? '0' : '') + month + '/' + d.getFullYear();
		console.log(output_INI);

		$('#datainicial').val(output_INI);
	...
	function procurarArtigo(){
		...
		if(!dataIni || !dataFim){
			...
			if(dataIni > dataFim){
				...
				var Conta_geral 	= $('#Conta_Geral').val(); // Varivel com o nome e Codigo
				...
				$.getJSON('modules/contabilidade/tabelacontabilidade.php',{'dataini':din,'datafim':fin,'cg':Conta_geral,'tipoPesquisa':tipoPesquisa},function(dataCab){
				$.getJSON('modules/contabilidade/total_contabilidade.php',{'datafim':fin,'cg':Conta_geral},function(dataCab2){ 
				...
				document.getElementById('tableCaption').innerText = 'Total:' + dataCab2[0].total ;
					...
					if (dataCab.length == 0){
						...
						for (var i=0; i<dataCab.length; i++){
							...
							var obj = new Array();
							obj[0] = dataCab[i].datadoc;
							obj[1] = dataCab[i].nrdiario;
							obj[2] = dataCab[i].nrlancto;
							obj[3] = dataCab[i].descmovcnt;
							obj[4] = dataCab[i].nrdocofic;
							obj[5] = dataCab[i].serie;
							obj[6] = dataCab[i].vld;
							obj[7] = dataCab[i].vlc;
						...
						$(document).ready(function() {
        					...
        					// Aplica o alinhamento inicial
								...
								$('#tabelaArtigos td:nth-child(5)').css('text-align', 'center');
								$('#tabelaArtigos td:nth-child(6)').css('text-align', 'center');
            					$('#tabelaArtigos td:nth-child(8)').css('text-align', 'right');
       					 		$('#tabelaArtigos td:nth-child(9)').css('text-align', 'right');
								
								 $('#tabelaArtigos td:nth-child(7)').each(function() { // Altere o número da coluna conforme necessário
							...
							// Formata como moeda em Euro
							$('#tabelaArtigos td:nth-child(7)').each(function() {
