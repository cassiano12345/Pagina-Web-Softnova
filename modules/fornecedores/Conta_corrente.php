		...
		<div id="contentMain">
			<div class="box span12">
				<div class="box-header" data-original-title="">
					<h2><i class="halflings-icon edit"></i><span class="break"></span>Conta corrente</h2>
				</div>
                                	...
                                	<div class="control-group" id="pesquisas">
										...
										<div class="controls">
											<select id="tipoPesquisa" >
                                                ...
                                                <option value="J">Justificações</option>
                                                <option value="3">Movimentos em Aberto (Não cobertos)</option>
											</select>
										</div>
									</div>
...
$script="<script type='text/javascript'>
    ...
    var autocomplete = $('#Conta_Geral').typeahead().on('keyup', function(ev) {
        ...
        ev.preventDefault();
			...
			if($('#typeahead').val().length >=1){
				...
				$.get('modules/fornecedores/listaForn.php', {'cliente': cliente} ,function(infoClientes){
    ...
    $('#Conta_Geral').on('input', function() {
        if ($('#Conta_Geral').val().length >= 2) {
            ...
            $.getJSON('modules/fornecedores/Lista_CG.php', {'exp': expr, 'data': ano }, function(infoArtigo) {
	...
	function procurarArtigo(){
		...
		if(!dataIni || !dataFim){
			...
			if(dataIni > dataFim){
				...
				var Conta_geral 	= $('#Conta_Geral').val();
				...
				$.getJSON('modules/fornecedores/tabelafornecedores.php',{'dataini':din,'datafim':fin,'cg':Conta_geral,'codigo':Codigo,'tipoPesquisa':tipoPesquisa},function(dataCab){
				$.getJSON('modules/fornecedores/total.php',{'datafim':fin,'cg':Conta_geral,'codigo':Codigo},function(dataCab2){ 
				...
				document.getElementById('tableCaption').innerText = 'Total:' + dataCab2[0].total ;
					...
					if (dataCab.length == 0){
						...
						for (var i=0; i<dataCab.length; i++){
							...
							obj[1] = dataCab[i].dvc;
							obj[2] = dataCab[i].nrdiario;
							obj[3] = dataCab[i].nrlancto;
